...
export default function Home() {
  ...
  const runAudit = useCallback(async (sourceCode?: string) => {
    const target = sourceCode || code;
  ...
  const handleFetched = (sourceCode: string, _name: string, meta: FetchedMeta) => {
    setCode(sourceCode);
    setFetchedMeta(meta);
    setFetchError("");
        ...
        <div style={{ maxWidth: 1440, margin: "0 auto", padding: "0 24px", height: 58, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
              <span style={{ fontFamily: "'Fraunces', Georgia, serif", fontSize: 17, fontWeight: 500, color: "white", letterSpacing: "-0.01em" }}>Hawkeye</span>
              <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase" as const, background: "rgba(37,99,168,0.5)", color: "#7EB3D8", border: "1px solid rgba(59,130,196,0.3)", borderRadius: 4, padding: "2px 6px" }}>Beta</span>
            </div>
          </div>
        ...
        <div style={{ maxWidth: 1440, margin: "0 auto", padding: "36px 24px 40px" }}>
          <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 40 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
              {[{ n: "40+", label: "Vulnerability\nclasses" }, { n: "<10s", label: "Average\naudit time" }, { n: "6", label: "Supported\nchains" }].map(s => (
      ...
      <main style={{ flex: 1, maxWidth: 1440, margin: "0 auto", width: "100%", padding: "20px 24px" }}>
        <div style={{ display: "flex", gap: 20, height: "calc(100vh - 250px)", minHeight: 560 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", width: "46%", minWidth: 0 }}>
            ...
            {/* Input mode tabs */}
            <div style={{ display: "flex", gap: 2, marginBottom: 12, background: "#F1F5F9", borderRadius: 10, padding: 3 }}>
              {([
                { id: "paste",   label: "Paste Code",     icon: <FileCode size={12} /> },
                { id: "address", label: "Contract Address", icon: <Link2 size={12} /> },
              ] as { id: InputMode; label: string; icon: React.ReactNode }[]).map(tab => (
                <button key={tab.id}
                  onClick={() => { setInputMode(tab.id); setFetchError(""); }}
                  style={{
                    flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6,
                    padding: "7px 12px", borderRadius: 8, border: "none", cursor: "pointer",
                    fontSize: 12, fontWeight: 600, transition: "all 0.15s",
                    background: inputMode === tab.id ? "white" : "transparent",
                    color: inputMode === tab.id ? "#0B1F3A" : "#64748B",
                    boxShadow: inputMode === tab.id ? "0 1px 3px rgba(11,31,58,0.08)" : "none",
                  }}>
                  {tab.icon}{tab.label}
                </button>
              ))}
            ...
            {inputMode === "address" && (
              <div style={{ marginBottom: 12 }}>
                ...
                {/* Fetched contract meta */}
                {fetchedMeta && (
                  <div style={{ marginTop: 8, padding: "10px 12px", borderRadius: 8, background: "#F4F9FD", border: "1px solid #BAD6EC", fontSize: 11, color: "#334155", display: "flex", flexWrap: "wrap", gap: 8 }}>
                    <span style={{ fontFamily: "monospace", color: "#2563A8" }}>{fetchedMeta.address.slice(0, 6)}…{fetchedMeta.address.slice(-4)}</span>
                    <span style={{ color: "#94A3B8" }}>·</span>
                    <span>{fetchedMeta.chainLabel}</span>
                    <span style={{ color: "#94A3B8" }}>·</span>
                    <span style={{ color: "#64748B" }}>{fetchedMeta.compiler}</span>
                    {fetchedMeta.proxy && (
                      <><span style={{ color: "#94A3B8" }}>·</span><span style={{ color: "#D97706", fontWeight: 600 }}>Proxy contract</span></>
                    )}
                  </div>
                )}
              </div>
            ...
            {inputMode === "paste" && (
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  {lineCount > 0 && <span style={{ fontSize: 11, fontFamily: "'JetBrains Mono', monospace", color: "#CBD5E1" }}>{lineCount} lines</span>}
            ...
            {/* Code box */}
            <div style={{
              flex: 1, position: "relative", borderRadius: 12, background: "white", overflow: "hidden",
              border: state === "scanning" ? "1px solid #3B82C4" : "1px solid #E2E8F0",
              boxShadow: "0 1px 3px rgba(11,31,58,0.06)", transition: "border-color 0.3s",
              opacity: inputMode === "address" && !code ? 0.5 : 1,
            }}>
              <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: 44, background: "#F8FAFC", borderRight: "1px solid #F1F5F9", display: "flex", flexDirection: "column", paddingTop: 16, overflow: "hidden", userSelect: "none" }}>
                {(code || "x\nx\nx\nx\nx\nx\nx\nx").split("\n").map((_, i) => (
                  <div key={i} style={{ textAlign: "right", paddingRight: 10, fontFamily: "'JetBrains Mono', monospace", fontSize: 10.5, lineHeight: "1.8", color: "#CBD5E1" }}>{i + 1}</div>
                ))}
              </div>
              <textarea
                ref={textareaRef}
                ...
                placeholder={inputMode === "address"
                  ? "// Source code will appear here after fetching…"
                  : "// Paste your Solidity contract here\n// or choose a sample above\n\npragma solidity ^0.8.20;\n\ncontract Example {\n    // Your code...\n}"}
                className="code-input"
                readOnly={inputMode === "address" && !fetchedMeta}
            ...
            {/* Run button — only show in paste mode */}
            {inputMode === "paste" && (
              <button
                onClick={() => runAudit()}
            ...
            {/* Address mode — show re-audit button if code already fetched */}
            {inputMode === "address" && code && state !== "scanning" && (
  <button
    onClick={() => runAudit()}
    className="btn-primary"
    style={{ marginTop: 10, width: "100%", padding: "14px", borderRadius: 12, fontSize: 14 }}>
    <Play size={13} fill="currentColor" /> Re-run Audit
  </button>
)}
          </div>
          ...
          <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10, height: 36 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <span style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase" as const, letterSpacing: "0.08em", color: "#94A3B8" }}>Audit Report</span>
              </div>
              {state === "done" && result && (
              ...
              <div style={{ height: "100%", overflowY: "auto", padding: 20 }}>
                ...
                {state === "idle" && (
                  <div style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 24, textAlign: "center" }}>
                    ...
                    <div>
                      <p style={{ fontFamily: "'Fraunces', Georgia, serif", fontSize: 15, fontWeight: 500, color: "#0B1F3A", marginBottom: 6 }}>Ready to audit</p>
                      <p style={{ fontSize: 13, color: "#94A3B8" }}>Paste code or enter a contract address to begin</p>
                    </div>
                    <div style={{ width: "100%", maxWidth: 300 }}>
                      <p style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase" as const, letterSpacing: "0.1em", color: "#CBD5E1", marginBottom: 10 }}>What we detect</p>
                ...
                {state === "scanning" && (
                  <div style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 24 }}>
                    ...
                    <div style={{ textAlign: "center" }}>
                      <p style={{ fontFamily: "'Fraunces', Georgia, serif", fontSize: 15, fontWeight: 500, color: "#0B1F3A", marginBottom: 6 }}>Analysing contract</p>
                      <p style={{ fontSize: 13, color: "#94A3B8", minHeight: 20, transition: "all 0.5s" }}>{SCAN_STEPS[scanStep]}</p>
                    </div>
